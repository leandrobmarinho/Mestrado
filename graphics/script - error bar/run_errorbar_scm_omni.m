...
titulo_extrator = {'SCM-A', 'SCM-G', 'SCM-L', 'SCM-S', 'GLCM', 'LBP', 'Statistical M.', 'SIFT-SURF'};

load('route_scm_omni.mat');
% Remove CM and Hu
valuesNormal = valuesNormal(:, [1:4 6 8 9]); errosNormal = errosNormal(:, [1:4 6 8 9]);
valuesRej = valuesRej(:, [1:4 6 8 9]); errosRej = errosRej(:, [1:4 6 8 9]);
for i = 1 : 7
    dataValues{i} = [valuesNormal(:,i) valuesRej(:,i)]*100;
    dataErros{i} = [errosNormal(:,i) errosRej(:,i)]*100;
end
dataValues{8} = [sift; surf]*100;
dataErros{8} = [sift_erro; surf_erro]*100;
...
for i = 0:7
    ...
    set(gca,'FontSize',sizeAxis) %tamanho axis
    set(l,'FontSize',sizeLegend); %tamanho da legenda
...
%     mycolor=[90/255 50/255 0;166/255 166/255 166/255;0.2 0.2 0.2;1 1 1];
    ...
    ylabel(labelY, 'FontSize', sizeLabelY, 'interpreter','latex')
    xlhand = get(gca,'ylabel');
    xlhand.FontSize = sizeLabelY;
