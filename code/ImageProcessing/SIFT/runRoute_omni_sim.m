addpath('vlfeat/toolbox');vl_setup; clear; close all; clc; p = path; path(p, '../'); path(p, '../../utils/');
...
%%
nameImgs = sprintf('%s_omni', env);
pathData = sprintf('/Users/WELL/Documents/Leandro/data/SIFT_%s_omni/desc_sift_%s_omni_', env);
...
%         [Y, ~, y_] = testSIFT(model, testData, numK, nnThreshold);
        [Y, ~, y_] = testSIFT_ind(model, testData, numK, nnThreshold, pathData);
