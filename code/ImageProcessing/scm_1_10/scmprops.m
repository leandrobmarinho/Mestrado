...
%SCMPROPS - Computes attributes from structural cooccurrence matrix (SCM) - v1.10 by Geraldo Ramalho aug/2016  
%
...
%         - tabela de conting?ncia (diagonal = alta depend?ncia)
%         - coeficiente de conting?ncia (CC=sqrt(X2/(X2+n)); n=total de medicoes)
%         - X2 = medida para a diferen?a entre valores observados e
%         esperados
...
for p = 1 : numSCM
  ...
  %%% out/2013 - permite uso com fun??o blockproc!!!
...
term1 = (r - mr) .* (c - mc) .* SCM(:); 
term2 = sum(term1);
Corr = term2 / (Sr * Sc); % = correla??o estat?stica
...
% OBS: o conceitualmente correto ? usar a SCM assimetrica!!!!
