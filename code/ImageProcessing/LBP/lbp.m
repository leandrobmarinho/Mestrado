...
%
function [X,Xn,options] = lbp(I,R,options)
...
code_img = lbp_(I,options.radius,options.samples,mapping,'');
[n1,n2]  = size(code_img);
[N,M]    = size(I);
Ilbp     = zeros(size(I));
i1 = round((N-n1)/2);
...
function result = lbp_(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.3.2
% Authors: Marko Heikkila and Timo Ahonen
